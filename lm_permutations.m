...
for perm_num = 1:n_perm
    ...
    for network_num = 1:length(network) % get permuted FC similarity changes
        ...
        for subj_num = 1:size(lme_tbl,1)/2
            subj_data_idx = find(~cellfun('isempty',strfind(lme_tbl_perm.subject,subj_name{subj_num}))); % get idx for the subject (two rows per subject)
            fcsim_diff(subj_num,network_num) = lme_tbl_perm{subj_data_idx(2),network_idx} - lme_tbl_perm{subj_data_idx(1),network_idx}; % FC similarity change
        end
        lm_tbl_perm = [lm_tbl_perm array2table(fcsim_diff(:,network_num),'VariableNames',{[network{network_num} '_intra_fc_sim_diff']})]; % add intranetwork FC similarity change to lm table
        ...
        lm_tbl_perm = [lm_tbl_perm array2table(fcsim_diff(:,network_num),'VariableNames',{[network{network_num} '_inter_fc_sim_diff']})]; % add internetwork FC similarity change to lm table
    end
